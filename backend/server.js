...
const db = new sqlite3.Database(':memory:', (err) => {
    if (err) {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS ocorrencias (
            id INTEGER PRIMARY KEY AUTOINCREMENT, protocolo TEXT NOT NULL UNIQUE, tipo_ocorrencia TEXT NOT NULL, data_inicio TEXT,
            hora_inicio TEXT, data_termino TEXT, hora_termino TEXT, local TEXT, prefixo TEXT, motivo TEXT, detalhe_falha TEXT,
            providencia TEXT, providencia_horario TEXT, providencia_sentido TEXT, providencia_veiculo_substituto TEXT,
            itinerario TEXT, desvio_realizado TEXT, pontos_perdidos INTEGER
        )`);
    }
});
...
app.post('/ocorrencias', (req, res) => {
    // --- MUDANÇA PRINCIPAL: GERANDO UM PROTOCOLO SIMPLES SEM CONSULTA AO BANCO ---
    const timestamp = Date.now();
    const protocolo = `${timestamp}`.slice(-5) + '/' + new Date().getFullYear();
    ...
    const sql = `INSERT INTO ocorrencias (protocolo, tipo_ocorrencia, data_inicio, hora_inicio, data_termino, hora_termino, local, prefixo, motivo, detalhe_falha, providencia, providencia_horario, providencia_sentido, providencia_veiculo_substituto, itinerario, desvio_realizado, pontos_perdidos) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
...
app.put('/ocorrencias/:id', (req, res) => {
    ...
    const sql = `UPDATE ocorrencias SET data_inicio = ?, hora_inicio = ?, data_termino = ?, hora_termino = ?, local = ?, prefixo = ?, motivo = ?, detalhe_falha = ?, providencia = ?, providencia_horario = ?, providencia_sentido = ?, providencia_veiculo_substituto = ?, itinerario = ?, desvio_realizado = ?, pontos_perdidos = ? WHERE id = ?`;
    db.run(sql, params, function (err) {
        if (err) {
